...
if(!msg.member.hasPermission('MANAGE_MESSAGES')) return; // permissions
...
if (reason.length > 100) {
    ...
    var warnEmbed = new Discord.MessageEmbed()
        .setColor('YELLOW')
        .setDescription(`**${user}** has been **Warned** | \`${userID}\``)
      await msg.channel.send(warnEmbed);


        const Channel = msg.guild.channels.cache.find(c => c.id === '1186800784071995422') //ADD YOUR CHANNEL id HERE
        ...
        .setColor('RED')
        .setTitle(`You were warned on Murshad\'s Server`)
        .addField("Server:", 'Murshad\'s Server')
        .addField("Reason:", `${reason}`)
        .addField("Punishment ID:", `\`${punishmentID}\``);
    try {
    user.send(embed)
