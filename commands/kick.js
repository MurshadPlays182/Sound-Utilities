...
exports.run = async(client, message, args) => {
    ...
    if(!message.member.hasPermission('KICK_MEMBERS')) return; // the permissions
    ...
    if(member){
        if(member.hasPermission('MANAGE_MESSAGES')) return message.reply('You can not kick this staff member');
    ...
    .setDescription("kicked Information Displayed Below:")
    .addField("Member kicked:", `${user}> **|** \`${user}\``)
    ...
    .setColor('RED')
    .setTitle(`You were kicked in Murshad\'s Server `)
    .addField("Server:", 'Murshad\'s Server') //delete Murshad server and write your server name
    .addField("Reason:", `${reason}`)
    .addField("Punishment ID:", `\`${punishmentID}\``)
    .addField("Time", time)
    try {
    ...
    .setColor('RED')
    .setDescription(`**<@${member.user.id}> **has been **Kicked** | \`${member.user.id}\``)
